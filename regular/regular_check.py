...
import csv
import jieba
from regular import char_count
...
def get_engset():
    ...
    # engset = set()
    global engset
    with open('../file/engdict.csv', 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        for i in reader:
            engset.add(i[0])
...
if __name__ == '__main__':
    ...
    print(res)
    # one_string = input('输入检测数据:')
    # size, interval, ratio, regular_index, one_pattern = input('输入参数空格隔开size ,interval, ratio, regular_index, one_pattern:').split()
    #
    # size = int(size)
    ...
    #
    # func_index = ['1', '2', '3', '4', '5']
    # regular_index = list(regular_index)
    # regular_index = list(set(regular_index) & set(func_index))
    # regular_index = [int(regular_index[i]) for i in range(len(regular_index))]
    #
    # check(one_string, size, interval, ratio, regular_index, one_pattern)
    #
    # os.system("pause")
